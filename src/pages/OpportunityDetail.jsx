...
import {
  ArrowLeft, MapPin, Calendar, Clock, Users, DollarSign, Trophy,
  BookOpen, Heart, Share2, ExternalLink, ShieldCheck, Zap,
  ChevronRight, Award, MessageSquare, Globe, Building2, Rocket
} from 'lucide-react';
    ...
    const fetchOpportunity = async () => {
      try {
        ...
        if (data && data._id) {
          ...
          const mockItem = mockOpportunities.find(opp => opp.id.toString() === id || opp._id === id);
        ...
        if (user && user.applications) {
          const alreadyApplied = user.applications.some(app =>
            (app.opportunityId?._id || app.opportunityId) === id
          );
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center">
        ...
        <p className="mt-6 text-slate-500 font-black uppercase tracking-widest text-xs">Retrieving details...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white p-12 rounded-[3rem] shadow-xl text-center max-w-lg border border-slate-100">
          ...
          <h2 className="text-3xl font-black text-slate-900 mb-4 tracking-tight">Opportunity Missing</h2>
          <p className="text-slate-500 mb-8 font-medium">The link might be expired or the opportunity has been removed by the host. Check out other trending drops instead.</p>
          <button
            onClick={() => navigate('/')}
      ...
      {showSuccess && (
        <div className="fixed top-12 left-1/2 -translate-x-1/2 z-[100] w-full max-w-md px-4 animate-in slide-in-from-top-8 duration-500">
          <div className="bg-slate-900 border border-emerald-500/30 p-6 rounded-[2rem] shadow-2xl flex items-center space-x-4">
            ...
            <div>
              <p className="text-white font-black uppercase tracking-widest text-[10px]">Transmission Success</p>
              <p className="text-slate-400 text-xs font-medium">Your credentials have been securely pushed to {opportunity.company}.</p>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl">
              <div className="flex flex-wrap items-center gap-3 mb-6">
                <span className={`px-4 py-1.5 ${theme.bg} text-white text-[10px] font-black uppercase tracking-widest rounded-full shadow-xl`}>
                  {opportunity.type}
                </span>
              ...
              <div className="flex items-center text-slate-300">
                ...
                <div>
                  <span className="block text-xs font-black uppercase tracking-widest text-slate-400">Hiring Partner</span>
                  <span className="text-xl font-bold text-white">{opportunity.company}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                { icon: Calendar, label: 'Deadline', value: new Date(opportunity.deadline).toLocaleDateString() },
                {
                  icon: opportunity.type === 'hackathon' ? Trophy : DollarSign,
                  label: opportunity.type === 'hackathon' ? 'Prize Pool' : 'Compensation',
                  value: opportunity.type === 'internship' ? (opportunity.salary || 'Competitive') : (opportunity.prizes || opportunity.price || 'Free')
                }
              ].map((stat, i) => (
                <div key={i} className="bg-white p-6 rounded-[1.5rem] border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                  <div className={`w-10 h-10 ${theme.light} rounded-xl flex items-center justify-center mb-4`}>
                  ...
                  <span className="block font-bold text-slate-800 text-sm truncate">{stat.value}</span>
                </div>
              ))}
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] p-8 md:p-12 border border-slate-100 shadow-sm space-y-12">
              <section>
                ...
                <div className="mt-8 p-6 bg-slate-50 rounded-3xl flex items-start">
                  <Award className={`w-6 h-6 ${theme.text} mr-4 shrink-0 mt-1`} />
                  <div>
                    <h4 className="font-black text-slate-800 uppercase tracking-widest text-xs mb-2">Key Value Proposition</h4>
                    <p className="text-sm text-slate-500 font-medium">This opportunity is highly rated by past participants for its mentorship quality and industry recognition. Perfect for students looking to scale their technical expertise.</p>
                  </div>
                </div>
              </section>
              ...
              <section>
                <div className="flex items-center space-x-3 mb-8">
                  <div className={`w-1.5 h-8 ${theme.bg} rounded-full`}></div>
                  <h2 className="text-2xl font-black text-slate-900 tracking-tight">Prerequisites & Stack</h2>
                </div>
                <div className="flex flex-wrap gap-3">
                  {opportunity.requirements?.map((req, index) => (
                    ...
                    >
                      ...
                      <span className={`text-sm font-black uppercase tracking-tight ${theme.text}`}>{req}</span>
                    </div>
                  ))}
                </div>
              </section>
              ...
              <section className="pt-8 border-t border-slate-50">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 uppercase tracking-[0.2em] font-black text-[10px] text-slate-400">
                  <div className="flex items-center">
                    ...
                    Drop Live Since: {new Date(opportunity.posted).toLocaleDateString()}
                  </div>
                  <div className="flex items-center">
                    <Users className="w-4 h-4 mr-2" />
                    Participation Index: {opportunity.applicants || opportunity.participants || opportunity.enrolled || 0} Activations
                  </div>
                </div>
              </section>
            </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-6">
              <div className="bg-slate-900 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group">
                ...
                <h3 className="text-xl font-black text-white mb-6 tracking-tight relative z-10">Application Hub</h3>

                <div className="space-y-4 relative z-10">
                  <button
                    onClick={handleApply}
                    disabled={isApplied || applying}
                    className={`w-full py-5 px-6 rounded-2xl font-black uppercase tracking-widest text-sm transition-all flex items-center justify-center gap-3 ${isApplied
                  ...
                  >
                    ...
                    {applying
                      ? 'Processing...'
                      : isApplied
                        ? (opportunity.type === 'internship' ? 'Applied Successfully' : (opportunity.type === 'course' ? 'Enrolled' : 'Registered'))
                        : (opportunity.type === 'internship' ? 'Initiate Application' : (opportunity.type === 'course' ? 'Start Learning' : 'Claim Your Spot'))
                    }
                  </button>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      onClick={handleSave}
                      className={`flex items-center justify-center gap-2 py-4 rounded-2xl border-2 font-black uppercase tracking-widest text-[10px] transition-all ${isSaved
                ...
                <div className="mt-10 pt-8 border-t border-white/10">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center bg-white/5 p-4 rounded-2xl hover:bg-white/10 transition-all border border-white/5 group/row">
                      <span className="text-white/60 text-xs font-bold">Offer Status</span>
                      <span className="text-emerald-400 font-black uppercase text-[10px] animate-pulse">Accepting</span>
                    </div>
                    <div className="flex justify-between items-center bg-white/5 p-4 rounded-2xl hover:bg-white/10 transition-all border border-white/5">
                      <span className="text-white/60 text-xs font-bold">Difficulty</span>
                      <span className="text-white font-black uppercase text-[10px]">Moderate</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm">
                <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-6">About the Host</h4>
                <div className="flex items-center mb-6">
                  <div className={`w-14 h-14 ${theme.light} rounded-2xl flex items-center justify-center mr-4 border ${theme.border}`}>
                    <Building2 className={`w-7 h-7 ${theme.text}`} />
                  </div>
                  <div>
                    <span className="block font-black text-slate-900 tracking-tight">{opportunity.company}</span>
                    <a href="#" className="text-indigo-600 text-xs font-bold flex items-center hover:underline">
                ...
                <p className="text-slate-500 text-xs font-medium leading-relaxed italic border-l-2 border-slate-100 pl-4">
                  "Pioneering digital experiences through innovation and student collaboration since 2018."
                </p>
